...
class DefaultReproduction(DefaultClassConfig):
    ...
    def compute_spawn(adjusted_fitness, previous_sizes, pop_size, min_species_size):
        ...
        spawn_amounts1 = norm * np.cumsum(spawn_amounts)
        spawn_amounts = np.round(norm * np.cumsum(spawn_amounts)).astype(int)
        spawn_amounts[-1] = pop_size
        spawn_amounts = np.diff(spawn_amounts, prepend=0)
        spawn_amounts = np.maximum(min_species_size, spawn_amounts)
        spawn_amounts[-1] = pop_size - spawn_amounts[:-1].sum()

        return spawn_amounts.tolist()
